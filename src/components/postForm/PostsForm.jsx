...
const handleAddPost = (event) => {
  ...
  const selectedAuthor = authors.find((author) => author.id === newPost.authorId);

  if (selectedAuthor) {
    const postWithAuthor = { ...newPost, authorId: selectedAuthor.id, author: selectedAuthor.name };
    ...
    <div className={styles.formContainer}>
      ...
      <form className={styles.form}>
        <select
          ...
          >
            {authors.map((author) => (
              <option style={{background:'gray'}} key={author.id} value={author.id}>
                {author.name}
              </option>
            ))}
          </select>
